...
const Field = ({ icon: Icon, label, name, value, editing, onChange, type = "text", textarea = false, isDoctor = false }) => {
  const handleChange = isDoctor
    ? (e) => onChange({ target: { name, value: e.target.value } })
    : onChange;
...
const ProfilePatient = () => {
  ...
  const handleDoctorInputChange = (e) => {
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, doctor: { ...prev.doctor, [name]: value } }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const fields = ["first_name","last_name","email","phone","gender","country","county","city","address","birth_date","status"];
      fields.forEach(f => { if (formData[f]) fd.append(f, formData[f]); });
                ...
                {editing ? (
                  <div className="flex gap-2 justify-center sm:justify-start">
                    <input type="text" name="first_name" value={formData.first_name || ""} onChange={handleInputChange}
                      className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm font-semibold text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#1C398E]/30 w-36"
                ...
                <div className="flex items-center gap-2 mt-1.5 justify-center sm:justify-start flex-wrap">
                  ...
                  {editing ? (
                    <input type="email" name="email" value={formData.email || ""} onChange={handleInputChange}
                      className="px-3 py-1 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-600 focus:outline-none focus:ring-2 focus:ring-[#1C398E]/30 w-52"
                      placeholder="Email" />
                  ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
            <div className="flex flex-col items-center text-center mb-5">
              ...
              <p className="text-sm text-gray-400">This action is permanent and cannot be undone. All your data will be removed.</p>
            </div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-300 mb-4"
