...
const DoctorReviewsSection = ({ doctorId }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch(`${API}/doctor-reviews/${doctorId}`).then(r => r.json()),
      fetch(`${API}/doctor-reviews/${doctorId}/summary`).then(r => r.json()),
    ])
    ...
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-bold uppercase tracking-widest text-gray-400">Patient Reviews</h2>
        </div>
        {summary && summary.total_reviews > 0 && (
          <div className="flex items-center gap-2">
            <span className="text-lg font-bold text-gray-800">{summary.average_stars}</span>
      ...
      {loading ? (
        ...
        <div className="text-center py-8">
          ...
          <p className="text-xs text-gray-300 mt-1">Reviews appear after completed appointments</p>
        </div>
      ) : (
        <div className="space-y-3">
          {reviews.map(r => (
            <div key={r.id} className="p-4 border border-gray-100 rounded-xl hover:border-gray-200 transition-all">
              ...
              {r.message && <p className="text-sm text-gray-600 leading-relaxed pl-10">{r.message}</p>}
            </div>
          ))}
        </div>
      )}
...
const Field = ({ icon: Icon, label, name, value, editing, onChange, type = "text", textarea = false, isDoctor = false }) => {
  const handleChange = isDoctor ? (e) => onChange({ target: { name, value: e.target.value } }) : onChange;
  return (
    <div className="group">
      ...
      {editing ? (
        name === "gender" ? (
          ...
          <textarea name={name} value={value} onChange={handleChange} rows={3} placeholder={label}
          ...
          <input type={type} name={name} value={value} onChange={handleChange} placeholder={label}
            className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#1C398E]/30 focus:border-[#1C398E]/50 transition-all" />
        )
      ) : (
...
const Profile = () => {
  ...
  // Fetch doctor_id dacă e doctor
  useEffect(() => {
    if (user?.role !== "doctor") return;
    const token = localStorage.getItem("access_token");
    fetch(`${API}/doctors/me`, { headers: { Authorization: "Bearer " + token } })
      .then(r => r.json())
      .then(data => setDoctorId(data.doctor_id))
      .catch(() => {});
  }, [user]);

  const handleInputChange = (e) => { const { name, value } = e.target; setFormData(prev => ({ ...prev, [name]: value })); };
  const handleDoctorInputChange = (e) => { const { name, value } = e.target; setFormData(prev => ({ ...prev, doctor: { ...prev.doctor, [name]: value } })); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.role === "doctor" && formData.doctor) {
        if (formData.doctor.specialty)     fd.append("specialty", formData.doctor.specialty);
        if (formData.doctor.description)   fd.append("description", formData.doctor.description);
        if (formData.doctor.accreditation) fd.append("accreditation", formData.doctor.accreditation);
      }
    ...
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-4 pb-10">
        ...
        {formData?.role === "doctor" && formData?.doctor && (
          <Section title="Professional" icon={FiBriefcase}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Field icon={FiBriefcase} label="Specialty" name="specialty" value={formData.doctor.specialty || ""} editing={editing} onChange={handleDoctorInputChange} isDoctor />
              <Field icon={FiInfo} label="Accreditation" name="accreditation" value={formData.doctor.accreditation || ""} editing={editing} onChange={handleDoctorInputChange} isDoctor />
              <div className="md:col-span-2">
                <Field icon={FiInfo} label="Description" name="description" value={formData.doctor.description || ""} editing={editing} onChange={handleDoctorInputChange} textarea isDoctor />
              </div>
            </div>
          </Section>
        )}

        {/* Patient reviews — doar pentru doctori, după ce doctorId e încărcat */}
        {user?.role === "doctor" && doctorId && (
          <DoctorReviewsSection doctorId={doctorId} />
        )}

        {/* App feedback — toate rolurile */}
        <AppFeedbackCard />
