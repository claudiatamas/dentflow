...
const TYPE_CONFIG = {
  appointment_status: { icon: Calendar, color: 'text-blue-500', bg: 'bg-blue-50' },
  appointment_reminder: { icon: Clock, color: 'text-indigo-500', bg: 'bg-indigo-50' },
  new_message: { icon: MessageCircle, color: 'text-green-500', bg: 'bg-green-50', route: () => `/chat_users` },
  feedback_request: { icon: Star, color: 'text-amber-500', bg: 'bg-amber-50', route: () => null },
  medical_record_updated: { icon: FileText, color: 'text-purple-500', bg: 'bg-purple-50', route: () => `/medical_records` },
  ticket_update: { icon: Ticket, color: 'text-cyan-500', bg: 'bg-cyan-50', route: () => `/help_user` },
  low_stock: { icon: Package, color: 'text-red-500', bg: 'bg-red-50', route: () => `/stock` },
  new_ticket: { icon: Ticket, color: 'text-orange-500', bg: 'bg-orange-50', route: () => `/help_admin` },
  weekly_signup_summary: { icon: Users, color: 'text-teal-500', bg: 'bg-teal-50', route: () => `/admin_users` },
};

const DEFAULT_CONFIG = { icon: Bell, color: 'text-gray-500', bg: 'bg-gray-50', route: () => null };
...
const NotifItem = ({ notif, onRead, onDelete, onNavigate, onFeedbackRequest, userRole }) => {
  ...
  const handleClick = () => {
    ...
    // feedback_request → deschide FeedbackModal în loc de navigare
    if (notif.type === 'feedback_request') {
      onFeedbackRequest(notif);
      return;
    }
...
const NotificationBell = () => {
  ...
  const navigateTo = (route) => { setOpen(false); window.location.href = route; };

  // Deschide FeedbackModal la click pe notificare feedback_request
  const handleFeedbackRequest = (notif) => {
    // Extrage numele doctorului din body-ul notificării
    const match = notif.body?.match(/with (.+?) is/);
    setFeedbackModal({
      open: true,
      appointmentId: notif.entity_id,
      doctorName: match ? match[1] : null,
    });
    setOpen(false);
  };

  const closeFeedbackModal = () =>
    setFeedbackModal({ open: false, appointmentId: null, doctorName: null });
  ...
  return (
    <>
                    ...
                    <>
                      ...
                      {unread.map(n => (
                        <NotifItem key={n.id} notif={n} onRead={markRead} onDelete={deleteNotif}
                          onNavigate={navigateTo} onFeedbackRequest={handleFeedbackRequest} userRole={userRole} />
                      ))}
                    </>
                  )}
                  {read.length > 0 && (
      ...
      {/* FeedbackModal — randat direct pe body via portal, centrat corect */}
      {feedbackModal.open && ReactDOM.createPortal(
        <FeedbackModal
